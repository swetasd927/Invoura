      ...
      if (mounted) {
        setInvoice((prev) => {
          ...
          return {
            ...base,
            fromBusinessName:
              !base.fromBusinessName?.trim()
                ? serverProfile.businessName
                : base.fromBusinessName,
            fromEmail:
              !base.fromEmail?.trim() ? serverProfile.email : base.fromEmail,
            fromAddress:
              !base.fromAddress?.trim()
                ? serverProfile.address
                : base.fromAddress,
            fromPhone:
              !base.fromPhone?.trim() ? serverProfile.phone : base.fromPhone,
            fromVat:
              !base.fromVat?.trim() ? serverProfile.vat : base.fromVat,
            logoDataUrl:
              base.logoDataUrl ||
...
//       const headers = {Accept: "application/json"};
//       if(token) headers["Authorization"] = `Bearer ${token}`;
//       const res = await fetch(`${API_BASE}/api/invoice?invoiceNumber = ${encodeURIComponent(candidate)}`, {
//         method: "GET",
//         headers,
//       });
//       const json = await res.json().catch(() => null);
