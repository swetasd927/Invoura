...
const StatusBadge = ({ status = "", size = "default", showIcon = true }) => {
  ...
  return (
    <div className={`inline-flex items-center ${sizeClasses[size]} rounded-full font-medium
     ${config.bg} ${config.text} border ${config.border} transition-all duration-300 ease-out
     hover:scale-105 hover:shadow-sm group relative overflow-hidden`}>
        <div className={`absolute inset-0 bg-linear-to-r ${config.gradient} 
